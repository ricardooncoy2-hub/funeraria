...
### 2.2 Logo simplificado (espacios realmente reducidos)
El logo completo en PNG (`apps/admin/public/logo-minaya.png`) se usa en todo encabezado del admin donde entre razonablemente (header del sidebar expandido, overlay mobile, login, footer del sitio público, header del sitio público, documentos/PDFs exportados) — no hace falta simplificarlo solo porque el contenedor mida ~56px de alto, ese espacio alcanza.

La única excepción real es el **sidebar colapsado a solo íconos** (64px de ancho, SKILL.md §7): ahí ni el logo completo ni el wordmark de texto entran con margen legible, así que se usa una sola letra **"M"** en `font-semibold`, azul de marca. No inventar un isotipo recortado de la cruz sin tener el asset — usar la letra hasta que exista un recorte oficial.
...
- **Sidebar** con ítems organizados en grupos por proceso de negocio: *Ventas* (Clientes, Cotizaciones, Ventas, Servicios, Planes), *Finanzas* (Financiamiento, Cuentas por cobrar, Pagos, Caja), *Compras e inventario* (Proveedores, Compras, Inventario, Transferencias, Productos), *Administración* (Sedes, Usuarios y roles, Auditoría), más Dashboard y Reportes sueltos (sin grupo, al inicio y al final respectivamente). **Los ítems se muestran u ocultan según los permisos del usuario, cada uno de forma individual** — nunca se muestra un ítem que llevará a un 403 (defensa en UX, la seguridad real vive en backend). El logo vive en el encabezado del propio sidebar (logo completo si está expandido, letra "M" si está colapsado a solo íconos — §2.2), no en el topbar.
- **La agrupación es solo presentación/orden, nunca control de acceso.** No existe (ni debe crearse) un permiso "de grupo": un grupo se muestra si al menos uno de sus ítems pasa el filtro de permiso de ese ítem individual. Un mismo grupo puede verse con distinta cantidad de ítems según el rol — eso es esperado y correcto, no un bug a corregir agregando lógica de permisos al grupo.
- **Grupos como acordeón:** cada grupo con rótulo es una sección plegable (encabezado en `text-xs font-semibold uppercase tracking-wide text-neutral-500` + chevron `size-4` que rota `-rotate-90` al cerrar, `hover:bg-brand-200 hover:text-neutral-700` igual que un ítem de menú — es un control interactivo, no solo texto). Si el grupo contiene la ruta activa, el rótulo cambia a `text-brand-700` (así el usuario sabe en qué sección está aunque el ítem activo no esté a la vista). **Expandido por defecto** — el principio 4 (§1, "reducir pasos, no ocultarlos") aplica también al menú: el acordeón es para que el usuario *pueda* achicar secciones que no usa, no para que el sistema empiece escondiéndolas. Si la ruta activa está dentro de un grupo colapsado manualmente, ese grupo se fuerza a abierto (nunca esconder dónde está parado el usuario).
- **Separación entre grupos:** `mt-3` (12px) entre bloques en modo expandido — el rótulo ya basta para distinguir un grupo del siguiente, no hace falta línea divisoria. Los ítems sueltos (Dashboard, Reportes) y el sidebar en modo solo-íconos (colapsado, 64px) no llevan acordeón ni rótulo — se listan planos, pero en modo solo-íconos sí se agrega un separador (`border-t border-brand-200` + `mt-2 pt-2`) entre bloques, porque ahí no hay texto que cumpla esa función.
- **Fondo del chrome de navegación:** sidebar y topbar usan `bg-brand-100` + `border-brand-200` (no gris, no `bg-white`) — un azul de marca muy pálido que da identidad institucional a todo el panel sin competir con el azul saturado de botones/enlaces/foco, que sigue siendo el único acento "fuerte" con significado (§1.5). Se evaluaron y descartaron dos alternativas antes de esta: (a) gris liso (`neutral-200`) — funciona pero no aporta identidad de marca; (b) azul sólido/oscuro (`brand-600`/`brand-700` con texto claro) — da más presencia de marca pero exige rediseñar todos los estados de texto para contraste sobre fondo oscuro, y usa el mismo tono que ya significa "acción" en el resto de la app, diluyendo esa señal. `brand-100` es lo bastante pálido para que el texto oscuro existente (`neutral-700`/`neutral-500`/`neutral-950`) siga funcionando sin cambios de contraste. Como consecuencia, todo estado `hover` de un control dentro de ese chrome (ítem de menú, encabezado de grupo, botón de colapsar, hamburguesa, menú de usuario) usa `hover:bg-brand-200` — un fondo y su propio hover nunca pueden compartir tono, quedaría invisible. Los elementos tipo control que sí deben destacar sobre ese fondo (el selector de sede, el avatar de usuario, un dropdown flotante) se mantienen en `bg-white`, igual que cualquier input — **cuidado con reusar el mismo `brand-100`/`brand-200` para un elemento hijo del chrome** (p. ej. un badge o avatar): al compartir tono con su propio fondo, se vuelve invisible.
- **Topbar** fijo: botón de menú hamburguesa a la izquierda (solo mobile, el logo ya vive en el sidebar), selector de sede activa al centro/derecha (obligatorio si el usuario tiene más de una sede autorizada), menú de usuario (avatar/iniciales + nombre + logout) a la derecha.
