...
import { type ServiceItem, deactivateService, fetchServices } from "@/lib/api/services";
import { hasPermission, useAuthStore } from "@/lib/auth/auth-store";
import { toast } from "@/lib/toast/toast-store";

const columnHelper = createTableColumns<ServiceItem>();
...
export default function ServiciosPage() {
  ...
  const [editing, setEditing] = useState<ServiceItem | undefined>(undefined);
  const [deactivating, setDeactivating] = useState<ServiceItem | null>(null);

  const query = useQuery({
    queryKey: ["servicios", { q, page }],
    queryFn: () => fetchServices({ q: q || undefined, page }),
  });
  ...
  async function handleDeactivate() {
    ...
    try {
      ...
      toast.danger(error instanceof ApiError ? error.message : "No se pudo desactivar el servicio.");
      throw error;
    }
  }
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-semibold text-neutral-950">Servicios</h1>
          <p className="mt-1 text-sm text-neutral-700">Servicios funerarios (RF-031).</p>
        </div>
        {canManage && (
      ...
      <DataTable
        ...
        emptyTitle={q ? "No se encontraron servicios" : "Aún no hay servicios registrados"}
      ...
      {formOpen && (
        <ServiceFormModal open={formOpen} onOpenChange={setFormOpen} service={editing} onSuccess={refetch} />
      )}
