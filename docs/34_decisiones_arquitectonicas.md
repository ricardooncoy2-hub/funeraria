...
## ADR-020 — MariaDB en red Docker `internal: true` dedicada
- **Problema:** con `mariadb` como un servicio más del `docker-compose.yml` sin publicar su puerto al host, cualquier otro contenedor del mismo stack (`web`, `admin`, `nginx`) igual comparte red con la base de datos por defecto — "no publicar el puerto" evita el acceso *desde fuera del host*, pero no acota qué *contenedores* pueden alcanzarla.
- **Alternativas:** (a) mantener todo en la red `default` implícita, solo sin publicar el puerto de MariaDB (estado previo); (b) **red `backend` dedicada, `internal: true`** (sin ruta a internet ni a ninguna otra red), con solo `api` conectado a ella además de `default`; (c) compose separado para MariaDB con esa misma red como `external`, en una carpeta/ciclo de vida aparte del resto del stack.
- **Decisión:** (b), todo en el mismo `docker-compose.yml`. `mariadb` usa `container_name: funeraria-mariadb`, se conecta únicamente a la red `backend` (`internal: true`), y persiste vía bind mount a `/srv/funeraria/mariadb` en el host (no un volumen nombrado de Docker). `api` es el único servicio conectado tanto a `backend` como a `default` — `web`/`admin`/`nginx` no tienen ruta de red hacia la base de datos. Se descartó (c): un compose separado obliga a coordinar manualmente el `depends_on` entre archivos/proyectos distintos (Compose no puede expresarlo) y no aporta beneficio adicional sobre la red `internal: true` dentro del mismo archivo para el tamaño de este proyecto.
- **Justificación:** principio de mínimo privilegio de red — solo el servicio que realmente habla con la base de datos puede alcanzarla, ni siquiera `nginx`; el bind mount a una ruta fija del host mantiene los datos reales de clientes/ventas fuera del ciclo de vida de los contenedores (recrear `mariadb` no borra nada).
- **Consecuencias:** un único `docker-compose.yml` que seguir en despliegues (más simple operativamente, coherente con la simplicidad buscada para una PYME); `depends_on: condition: service_healthy` de `api` sobre `mariadb` funciona de forma nativa al estar en el mismo archivo.
